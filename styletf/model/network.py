...
class StyleTF(Model):
    ...
    def __init__(self, style_layer, content_layer):
        ...
        self.styletf_model = Model(inputs=self.vgg_model.input, 
                                   outputs=[self.vgg_model.get_layer(name).output for name in (self.style_layer + self.content_layer)])
    ...
    def __call__(self, inputs):
        ...
        outputs = self.styletf_model(preprocessed)

        style_outputs = outputs[:len(self.style_layer)]  # Extract style outputs
        ...
        content_outputs = outputs[len(self.style_layer):]  # Extract content outputs
